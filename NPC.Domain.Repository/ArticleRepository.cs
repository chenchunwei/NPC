...
namespace NPC.Domain.Repository
{
    public class ArticleRepository : AbstractNhibernateRepository<Guid, Article>
    {
        ...
        public IList<Article> Query(NPC.Domain.Models.Articles.ArticleQueryItem queryItem)
        {
            ...
            var query = Session.CreateSQLQuery(_nestSqlBuilder.BuilderRecord(string.Format(tempString, "distinct a.*", ""), "Order by OrderSort desc, DateOfCreate desc"));
        ...
        public IList<Article> GetTopNWithPic(Guid unitId, Guid categoryId, int picTopN, int normalTopN)
        {
            ...
            return Session.CreateSQLQuery(string.Format(@"select * from (select top {0} a1.* from Articles a1 join ArticleCategories ac1 on a1.ArticleCategoryId=ac1.Id  
                where a1.ArticleCategoryId=:ArticleCategoryId and a1.IsDelete=0 and a1.UrlOfCoverImage is not Null and a1.IsShow=1 and ac1.UnitId=:UnitId Order by a1.OrderSort desc,a1.DateOfCreate Desc) as t1 
                union 
                ...
                and a3.IsDelete=0 and a3.IsShow=1 and ac3.UnitId=:UnitId Order by a3.OrderSort desc,a3.DateOfCreate Desc) and a2.IsDelete=0 and a2.IsShow=1 Order by a2.OrderSort desc,a2.DateOfCreate Desc) as t2", picTopN, normalTopN))
        ...
        public IList<Article> GetTopN(Guid unitId, Guid categoryId, int topN)
        {
            return Session.CreateSQLQuery(string.Format(@"select top {0} * from Articles a join ArticleCategories ac on a.ArticleCategoryId=ac.Id 
                where a.ArticleCategoryId=:ArticleCategoryId and a.IsDelete=0  and a.IsShow=1 and ac.UnitId=:UnitId  Order by a.OrderSort desc,a.DateOfCreate Desc", topN))
                .AddEntity(typeof(Article))
                .SetGuid("UnitId", unitId)
                .SetGuid("ArticleCategoryId", categoryId)
                .List<Article>();
        ...
        public IList<Article> GetTopNPic(Guid unitId, Guid categoryId, int topN)
        {
            return Session.CreateSQLQuery(string.Format(@"select top {0} * from Articles a join ArticleCategories ac on a.ArticleCategoryId=ac.Id 
                ...
                and a.IsDelete=0 and a.UrlOfCoverImage is not Null and a.IsShow=1 and ac.UnitId=:UnitId Order by a.OrderSort desc,a.DateOfCreate Desc", topN))
                .AddEntity(typeof(Article))
                .SetGuid("UnitId", unitId)
                .SetGuid("ArticleCategoryId", categoryId)
                .List<Article>();
